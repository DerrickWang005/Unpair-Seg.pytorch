...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ret = {}
        ret["input_shape"] = {
            k: v for k, v in input_shape.items() if k in cfg.MODEL.OVSEG.IN_FEATURES
        }
        ret["conv_dim"] = cfg.MODEL.OVSEG.CONVS_DIM
        ret["mask_dim"] = cfg.MODEL.OVSEG.MASK_DIM
        ret["norm"] = cfg.MODEL.OVSEG.NORM
        ret["transformer_dropout"] = cfg.MODEL.OVSEG.DROPOUT
        ret["transformer_nheads"] = cfg.MODEL.OVSEG.NHEADS
        ret["transformer_dim_feedforward"] = (
            1024  # use 1024 for deformable transformer encoder
        )
        ret["transformer_enc_layers"] = (
            cfg.MODEL.OVSEG.TRANSFORMER_ENC_LAYERS
        )  # a separate config
        ret["transformer_in_features"] = (
            cfg.MODEL.OVSEG.DEFORMABLE_TRANSFORMER_ENCODER_IN_FEATURES
        )
        ret["common_stride"] = cfg.MODEL.OVSEG.COMMON_STRIDE
        return ret
