...
def gather_no_grad(tensor):
    world_size = get_world_size()
    if world_size == 1:
        return tensor
    # get tensor shape from all rank
    rank = get_rank()
    if rank == 0:
        # gather shape
        gathered_length = [None for _ in range(world_size)]
        dist.gather_object(tensor.shape[0], gathered_length, dst=0)
        # gather tensor
        gathered_tensor = [
            torch.zeros(gathered_length[i], device=tensor.device)
            for i in range(world_size)
        ]
        dist.gather(tensor, gathered_tensor, dst=0)
    else:
        dist.gather_object(tensor.shape[0], dst=0)
...
def all_gather_with_grad(tensor):
    ...
    if world_size == 1:
        return tensor
    gathered_tensor = all_gather(tensor)
    gathered_tensor = torch.cat(gathered_tensor, dim=0)
    return gathered_tensor
